...
export default {
  ...
  methods: {
    ...
    searchProvider(id) {
      return this.api.private('/people/client-company', { params: id })
        .then(response => response.json())
        .then(data => {
          if (data.response) {
            if (data.response.success === false)
    ...
    searchProviderById(id) {
      ...
      if (searchBy.document) {
        if (searchBy.document.length !== 14)
          return;
      }

      if (searchBy.email) {
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(searchBy.email))
          return;
      }

      this.isSearching = true;

      this.searchProvider(searchBy)
        .then(data => {
