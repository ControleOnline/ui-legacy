    ...
    <q-dialog v-model="dialog">
      <q-card style="width: 700px; max-width: 80vw;">
        <q-card-section class="row items-center">
          <div class="text-h6">{{ $t('New User') }}</div>
          <q-space />
...
export default {
  ...
  methods: {
    // store method
    getItems() {
      let endpoint = `trainers/${this.id}/users`;
    ...
    isInvalid(key) {
      return val => {
        switch (key) {
          case 'email'   :
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val))
              return this.$t('Email is not valid');
          break;
          case 'password':
            if (!val || val.length < 6)
              return this.$t('Password must to be at least 6 characters long');
          break;
          case 'confirm' :
            if (this.item.password != this.item.confirm)
              return this.$t('Password is not the same');
          break;
          default:
            if (!(val && val.length > 0))
