<template>
  <q-card class="my-card q-pa-md row items-stretch" flat bordered>
    <div class="col-3 text-center">
      <div class="row items-center full-height">
        <q-icon color="green-14" size="md" name="text_snippet" />
      </div>
    </div>
    <div class="col-9">
      <div class="column">
        <div class="col-6 text-right text-uppercase">
          {{ $t('dashboard.enrollments') }} ({{ enrollment_count.total_count }})
        </div>
        <div class="col-6 text-right text-green-4 text-h6">
          {{ (enrollment_count.total_price || 0) | formatMoney }}
        </div>
      </div>
    </div>
    <q-inner-loading :showing="isLoading">
      <q-spinner-gears size="50px" color="primary" />
    </q-inner-loading>
  </q-card>
</template>

<script>
import configurable from './../mixins/configurable';
import dashboard    from './../mixins/dashboard';

export default {
  name  : 'DashboardEnrollment',
  mixins: [ configurable, dashboard ],

  data() {
    return {
      query: 'enrollment',
    }
  },

  computed: {
    enrollment_count() {
      return this.data.enrollment_count ? this.data.enrollment_count : {}
    }
  },
};
</script>