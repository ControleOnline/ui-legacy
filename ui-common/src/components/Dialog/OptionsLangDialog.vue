  ...
  <q-dialog>
    <div class=" full-width" style="max-height: 70%">
      ...
      <q-input
        ...
        color="secondary"
      />
      <div v-for="option in filteredLangs" :key="option.id" class="flex items-center">
        <q-btn
...
export default {
  ...
  computed: {
    filteredLangs() {
      ...
      return optionsLang.filter((lang) =>
        lang.label
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
          .toLowerCase()
    ...
    options: {
      get() {
        return this.$store.getters["User/getOptionsLang"];
      },
      set(value) {
        this.$store.commit("User/setOptionsLang", { optionsLang: value });
      },
    },
  },
