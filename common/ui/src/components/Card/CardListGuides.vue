      ...
      >
        ...
        >
          <q-img :src="img.src" class="full-height">
            <div
              class="text-bold absolute-bottom text-subtitle1 text-secondary
              text-right bg-positive color-label"
            >
              R$ {{ formatMoney(price) }}
            </div>

            <div class="profile-radius q-ma-sm flex profile-container items-center bg-positive">
              <q-avatar size="2.3rem">
                <img :src="image_profile">
              </q-avatar>
              <div class="q-px-sm">
                <p class="q-ma-none text-caption text-bold text-primary">{{ user_name }}</p>
                <div class="flex items-center">
                  <p class="q-ma-none text-caption text-bold text-primary">{{ assessment }}</p>
            ...
            <div
              class="full-width absolute-top bg-transparent
                flex justify-end favorite-container"
            >
              <q-btn
                v-if="!edit"
                flat
...
export default {
  ...
  methods: {
    favoriteItem() {
      this.$store.commit('Guides/favoriteClick', { index: this.index, favorite: !this.favorite });
    },
    formatMoney(value) {
